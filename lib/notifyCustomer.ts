...
import { buildPreparingMail, buildShippedMail, sendEmail } from '@/lib/email'
 ...
 * 出荷準備に入ったことを知らせる。
 *
 * CSV出力（倉庫へ出荷指示を渡した時点）と、管理画面の「準備中にする」の
 * どちらからも同じ経路を通す。片方だけ直すと、操作によって顧客に届く内容が
 * 変わってしまうため。
 *
 * 発送完了メールと同じく例外を投げない。メールが送れなくても
 * ステータスの更新やCSVの出力は成立させる。
 */
export async function sendPreparingMail(input: OrderMailInput): Promise<NotifyResult> {
  const none = (message: string): NotifyResult =>
    ({ sent: 0, failed: 0, skipped: 0, noRecipients: true, message })
  ...
  try {
    ...
    for (const r of list) {
      // 宛名は宛先ごとに変わるため使い回さず1件ずつ組み立てる
      const { subject, body } = buildPreparingMail({
        customerName: input.customerName,
        recipientLabel: r.label ?? '',
        orderNumber: input.orderNumber,
        productName: input.productName,
    ...
    // ここで投げるとCSV出力やステータス更新まで失敗する。絶対に外へ出さない
    console.error('[mail] 出荷準備メールの処理で例外が発生しました', e)
    return none('メール送信で想定外のエラーが起きました')
  }
}
